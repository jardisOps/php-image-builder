...
| **N6** | **Erster Push auf `headgent/phpcli`/`phpfpm` ist der einzige Punkt, an dem laufende Projekte Schaden nehmen könnten.** **Die Tag-Strategie liegt seit 2026-07-26 vor: `docs/TAG-STRATEGIE.md`** — Kanalmodell `next`/`stable`, Beförderung per Registry-Retag statt Rebuild, sieben Stufen nach Schadensradius, neun Entscheidungspunkte T1–T9. Belegt darin: **26 aktive Fundstellen hängen an genau einem Tag, `headgent/phpcli:8.3`**; `phpfpm`, `nginx`, `:latest` und 8.2 haben **null** Konsumenten. Bis zur Freigabe unverändert: kein `docker login`, kein `--push`, keine CI-Auslösung | Freigabe steht aus | **hoch** |
